...
- ~~规划区 bbox 需要重新评估~~ **已于 2026-09-16 处理**：规划区在现有数据范围内
  西移 20 km 至 `114.0465, 36.9110, 115.4045, 37.9890`，纳入太行山地，未重新采集数据。
  规划区西边界与数据覆盖西边界重合，西侧无缓冲，贴边链路遮挡计算偏乐观；
  如需严格处理，需补采西延约 20 km 的 DEM。
